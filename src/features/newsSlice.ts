...
export const fetchTopHeadlines = createAsyncThunk(
  ...
  async (
    ...
    const prevArticles = (thunkAPI.getState() as { news: NewsState }).news
      .articles;
    const newArticles = response.data.articles;
    const allArticles =
      page === 1 ? newArticles : prevArticles.concat(newArticles);
    return {
...
export const newsSlice = createSlice({
  ...
  reducers: {
    setSortBy: (
      state,
      action: PayloadAction<"popularity" | "relevancy" | "publishedAt">
    ) => {
